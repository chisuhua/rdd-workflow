...
- Markdown 链接更新：`improvement-approved.md` 134 个 `[...](improvements/X.md)` → `[...](.rddf/improvements/X.md)`
- 文档同步：`docs/proposal-{suggestions,approved}-format.md` 路径说明
- 测试 fixture 更新：`tests/fixtures/diseased-repo/improvement-suggestions.md` 等
...
**Out of Scope**：
- 修改 `improvement-suggestions.md` 的表头/格式
...
- **SC-1 新建提案**: GIVEN 用户调用 `add-improve` skill, WHEN skill 完成 5 段设计 + 用户批准, THEN `.rddf/improvements/<name>.md` 被创建（不是 `improvements/`），且 `improvement-suggestions.md` 表格中的链接指向新路径
- **SC-2 状态扫描**: GIVEN scan-state.sh 在 guide 入口被调用, WHEN 扫描改进提案状态, THEN 从 `.rddf/improvements/*.md` 读取，输出中显示 133 个文件
- **SC-3 设计审查**: GIVEN 用户调用 `guide-design`，待审提案存在, WHEN skill 读取提案内容做内容审查, THEN 读取 `.rddf/improvements/<name>.md` 成功，批准后写入 `improvement-approved.md` 时链接使用新路径
...
- **SC-5 文档链接**: GIVEN 用户在 IDE/浏览器打开 `improvement-approved.md`, WHEN 点击任一提案链接, THEN 跳转到 `.rddf/improvements/<name>.md`，**不**是 404
...
- `improvement-approved.md` 134 个链接用 `sed -i 's|](improvements/|](.rddf/improvements/|g'` 一次性更新
...
- 不修改 `improvement-suggestions.md` 的表头/格式
- 不重新编号 ADR
- 不修改 add-improve UX
...
- 更新 `docs/improvement-suggestions-format.md` 和 `proposal-approved-format.md` 路径示例
...
- **AC-3 improvement-approved.md 链接全部可解析**：134 个 `.rddf/improvements/...` 链接全部指向存在的文件
- **AC-4 skill 行为不变**：
