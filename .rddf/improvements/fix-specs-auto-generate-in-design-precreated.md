...
> **症状**：design-pre-created (D3) 模式生成的 change 缺 `specs/<sub>/spec.md`，导致 openspec v1.4+ 严格校验时回归门报"新增失败 1 个"，阻塞 archive。
> **根因**：`guide-design/scripts/generate_full_proposal.py` 仅生成 `proposal.md`，未生成 spec delta 文件。但 openspec validate v1.4 要求每个 change 必须有 `specs/<cap>/spec.md` 含 `## ADDED Requirements` + `#### Scenario:` blocks。
> **临时绕过**：本次会话手工补 specs/ + amend worktree commit + commit master + 跑 4 轮回归门（合计 ~32 分钟）才收敛。
...
**A. 自动化 specs/ 生成（`generate_full_proposal.py` 扩展）**:
...
- **现状痛点**：每次 design-pre-created change 在 ship 阶段被回归门 fail，强制手工补 specs/（30+ 分钟），污染 user experience，且脚本与人类决策混合容易出错
- **修复价值**：自动化消除回归门误报，将 ship 阶段耗时从 30+ 分钟降至正常 ~8 分钟（纯回归门时间），并消除"必须手工" 的技术债
- **Why now**: 2026-08-31 session 是 rdd-workflow 自身 dogfood 第一个完整 5 阶段流程，暴露此 bug。未来任何用户用 rdd-workflow 都会触发同样 bug。P0 而非 P1 因为它阻塞 ship 的最后一步（archive），且无 workaround（每次必须手工 + amend commit + commit master）。

## What Changes

- `skills/guide-design/scripts/generate_full_proposal.py`: 新增 `generate_spec_delta()` 函数（~80 LOC）
- `skills/guide-design/scripts/approve_proposal.sh`: 调用 `write_specs_file()` 步骤（~15 LOC bash）
- `tests/unit/test_generate_full_proposal.py`: 5 个新单元测试
- `tests/integration/test_specs_generation.bats`: 5 个新 bats 测试（new file）
...
- MUST: design 阶段批准时自动生成 `openspec/changes/<name>/specs/<sub>/spec.md`（含 ## ADDED Requirements + #### Scenario: blocks）
- MUST NOT: 修改 proposal.md 生成内容（D2 映射兼容）
- MUST NOT: 覆盖已存在的 specs/（幂等保护）
...
- [ ] 5 个单元测试 PASS（生成路径全覆盖）
- [ ] 5 个 bats 集成测试 PASS（端到端验证）
- [ ] `openspec validate` 对 design-pre-created 新 change 不报 "No deltas found"
- [ ] `./test.sh --full --regression` 对 design-pre-created 新 change 跑出 0 新增失败（无需手工介入）
- [ ] 幂等性验证：重复运行 `approve_proposal.sh` 不覆盖已有 specs/
- [ ] AGENTS.md / SKILL.md / docs/ 同步更新
- [ ] 历史 archived change 行为不变（openspec validate 不参与 archive/）