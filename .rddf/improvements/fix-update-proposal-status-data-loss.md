...
- `skills/propose/scripts/update_proposal_status.py`（`archive-update-proposal-status` 改进引入）在归档时更新 improvement-approved.md
- 会话复盘 2026-07-31 实测：归档 `fix-plan-deps-candidates-import-guard` / `fix-rddf-session-lifecycle-binding` / `fix-test-infrastructure-and-skill-registration` 3 个 change 后，improvement-approved.md 的"已实施"表从 83 条审计记录坍缩到 1 条（仅剩最后归档的 change）——**历史审计数据被静默破坏**
...
- **Out Scope**:
  ...
  - 不修改 improvement-approved.md 文件格式/schema
  - 不涉及 proposals-suggestions.md

## 关键场景

- GIVEN improvement-approved.md 已实施表有 N 条历史记录, WHEN 归档 1 个 change 调用 `update_proposal_status`, THEN 新条目正确插入表头之后, 且 N 条旧记录**全部保留**（当前：旧记录全部丢失）
...
- 已损坏的 improvement-approved.md 可用修复后的脚本重跑恢复（重新标记已归档 change）
