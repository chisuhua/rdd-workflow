...
当前 `approve_proposal.sh --manual --hub-issue <org/repo#N>` 强制人工先有 Hub Issue 才能 approve。但 approve 流程本应**自动**创建 Hub Issue（基于已审批的草稿），让人类无需重跑 `rddf rfc-create`。

依赖 P0 #3 `add-rfc-interview-flow` 提供的草稿机制 + schema 校验。
