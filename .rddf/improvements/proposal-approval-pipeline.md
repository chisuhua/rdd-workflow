...
- 当前 `improvement-suggestions.md` 是一个 474 行的单体 JSON 文件，承载双重职责：既是"待讨论的提案池"，又是"plan 阶段直接可用的创建输入"
- `guide-arch` 完全不参与提案的审查/审批流程（0 次引用 `improvement-suggestions.md`）
...
- **In Scope**:
  ...
  - `improvement-suggestions.md` 降级为 Markdown 表格索引，仅存链接+元数据
  - 新建 `improvement-approved.md` 作为 plan 阶段的批准提案索引
  - `guide-arch` 新增 Phase 5.5：逐文件审查 improvements/，批准/拒绝/延迟
  - `guide-plan` propose 从 `improvement-approved.md` 读链接（替代原 JSON 解析）
  - `propose` skill `propose_change.py` 中 `set_suggestion_status` 改为读取 improvements/ 目录其文件 frontmatter
  - `archive.sh` 归档后更新 `improvement-approved.md` 条目状态
...
- GIVEN 提案被批准, WHEN 写入 `improvement-approved.md`, THEN 仅添加一行链接无需复制内容
- GIVEN guide-plan propos, WHEN 读取 `improvement-approved.md`, THEN 按链接打开对应 `improvements/xxx.md` 完成 change 创建

## 技术约束

- MUST 每个 improvements/ 文件自包含完整 5-section 内容（架构依据、范围、关键场景、技术约束、验收标准）
- MUST `improvement-suggestions.md` 和 `improvement-approved.md` 仅做索引不嵌入内容
- MUST 提案文件不包含 status 字段（状态由所在索引文件决定）
- SHOULD 索引文件追加新条目时保持表格按优先级排序
- MUST NOT 在 `improvement-approved.md` 中复制提案内容（仅链接）

## 验收标准

- `improvements/` 目录存在，所有提案为独立 `.md` 文件
- `improvement-suggestions.md` 为 Markdown 表格索引，< 60 行
- `improvement-approved.md` 为 Markdown 表格索引，< 30 行
- `guide-arch` Phase 5.5 支持逐文件审批
- `guide-plan` propose 从 `improvement-approved.md` 读取
