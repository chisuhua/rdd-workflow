...
1. **协议版本**：使用 MCP v0.5+（2025-Q2 后稳定版本）
2. **传输层**：Stdio + Streamable HTTP 双支持
3. **认证**：GitHub PAT（fine-grained，scoped to `rdd-hub` repo only）
4. **错误处理**：MCP Server 不可达时回退到 REST API + 警告
5. **可观测性**：每次 MCP 调用记录到 `.rddf/state/.mcp-trace.jsonl`
