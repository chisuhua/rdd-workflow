---
优先级: P1
来源: 架构差距分析 ADR-0030 / multi-project-ai-collaborative-development Step 6
阶段: v2.2
分类: cross-repo-federation
类型: feature
依赖 ADR: ADR-0030, add-rdd-hub-cross-repo-federation, add-mcp-cross-repo-protocol
状态: 已批准 (2026-08-15)
---
**优先级**: P1 | **来源**: 架构差距分析 ADR-0030 / multi-project-ai-collaborative-development Step 6
...
1. **图算法**：使用 Kahn's algorithm 拓扑排序，输出 wave-based 推荐顺序
2. **缓存**：`cross-repo-deps-cache.json` 缓存扫描结果（TTL 3600s）
3. **可观测性**：跨仓库依赖图渲染为 Mermaid 格式
4. **Schema 兼容**：iteration.json v7 必须向后兼容 v6（缺字段时默认空数组）
