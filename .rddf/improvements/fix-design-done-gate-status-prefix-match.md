...
- design-done gate 检查遍历 `improvement-suggestions.md`，对每行 status 列做精确比较：
  ```bash
...
```bash
local pending=$(grep -E '^\s*\|\s*\[' "$PROJECT_ROOT/improvement-suggestions.md" 2>/dev/null | \
...
问题：`[ "$status" != "已批准" ]` 是精确字符串比较。但 `improvement-suggestions.md` 的状态列会带人工追加的决策后缀（如日期、理由），如 `延迟 (2026-08-28, 维持 v3.2 deferred 决策)` 或 `已批准 (2026-09-01)`。

**这不仅是理论风险**：improvement-suggestions.md 的状态列没有 schema 约束，用户/工具可以自由追加决策上下文（如 `已批准 (2026-08-30, 关联 phase-4)`）。任何带后缀的条目都会让 gate 误报。
...
- `tests/unit/test_design_done_gate.sh` 单元测试（bash，模拟 improvement-suggestions.md 不同状态变体）
...
- **不修改** `improvement-suggestions.md` 状态列格式（带后缀是合法用法，由 gate 适配而非限制用户）
...
- **GIVEN** `improvement-suggestions.md` 有一行状态 `延迟 (2026-08-28, 维持 v3.2 deferred 决策)`
...
- **GIVEN** `improvement-suggestions.md` 某行状态列空白（`| |`）
- **WHEN** gate 检查
...
- **MUST NOT**: 改 `improvement-suggestions.md` 的 schema/状态词汇（`已批准`/`已拒绝`/`延迟`/`待审查` 保持）
...
- [ ] 复测 `improvement-suggestions.md` 当前含 `延迟 (2026-08-28...)` 状态时 gate 通过
...
- [ ] 复测 `improvement-approved.md` 的 `已实施` 状态不受影响（那是 approve 后的归档状态，不走 design-done）
...
- **现状痛点**：`improvement-suggestions.md` 的状态列支持人工追加决策上下文（后缀），但 gate 用精确匹配，导致任何带后缀的有效决策被误报"无决策"。本次会话 1 个 `延迟 (2026-08-28...)` 就触发，需人工 Python 绕过。
...
- MUST: 与 `improvement-suggestions.md` 状态机（`已批准`/`已拒绝`/`延迟`/`待审查`）向后兼容
...
- [ ] `improvement-suggestions.md` 含 `延迟 (2026-08-28...)` 时 gate 通过（无需 Python 绕过）
