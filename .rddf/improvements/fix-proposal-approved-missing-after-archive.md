...
执行 `archive-cleanup-plan-files-extension` 完整 lifecycle (approve → plan → ship → archive) 后, dashboard 仍报该提案为 📋 待讨论。Root cause 是 **improvement-approved.md 的 plan-phase commit 漏档**:

1. **Design 阶段**: `approve_proposal.sh` patch `improvement-approved.md` (主表格) 写入新行 ✅
2. **Plan 阶段**: 我 (AI 编排者) `git add openspec/changes/<name>/` + `improvements/<name>.md` 但**忘了 `improvement-approved.md`** ❌
3. **Archive 阶段**: `mark_approved_completed` 查找主表格无对应条目 → 跳过 → `## 已实施` section 未更新 ❌
4. **Dashboard 读取**: `improvement-approved.md` 用 regex `r"\|\s*\[([^\]]+)\]\(improvements/"` 提取所有 approved 集合; `improvements/<name>.md` 文件存在 → filter 不命中 → 报 pending
...
26e30e9 feat(plan): add archive-cleanup-plan-files-extension change
   ...
   ❌  漏 add: improvement-approved.md (含 design 阶段 approve_proposal.sh 写入的行)
...
- **In Scope**:
  - `approve_proposal.sh` (design 阶段) 改进: 写 `improvement-approved.md` 后立即 `git add improvement-approved.md` + echo "已 git add" 提示
  - `guide-plan.md` Phase 2 fill 完成时: 检测 `improvement-approved.md` 是否脏, 若脏则强制 commit
  ...
  - dashboard `_lib/dashboard/__init__.py` 改进: pending 判定加 fallback — 即使 `improvement-approved.md` 没该名,若 `openspec/changes/archive/*-<name>/` 存在则跳过
  - 加 1 个 regression test (bats): "approve_proposal immediately stages improvement-approved.md"
  - 加 1 个 unit test: "dashboard filters archived changes from pending"

- **Out of Scope**:
  - 不修改 `improvement-suggestions.md` 同步逻辑 (那是 plan 阶段另一回事)
  - 不修 `iteration.json` 同步 (那是 deps 阶段任务)
  - 不动 `archive-cleanup-plan-files-extension` 已提交的 9 commits (history 已存在, 不做回填)

## 关键场景

- **GIVEN** AI 编排者运行 `approve_proposal.sh <name>`, **WHEN** patch 写入 `improvement-approved.md`, **THEN** 立即 `git add improvement-approved.md` 并退出提示
- **GIVEN** Plan 阶段完成 fill, **WHEN** `improvement-approved.md` 脏, **THEN** plan-done gate 提示 "请先 commit improvement-approved.md", 否则不写 plan-handoff
- **GIVEN** archive 阶段 `mark_approved_completed <name>`, **WHEN** 主表格找不到该名, **THEN** fallback 检测 archive/<date>-<name>/ 存在 → 强制 append 到 `## 已实施` 末尾
- **GIVEN** dashboard 读取 pending, **WHEN** 某名不在 `improvement-approved.md` 但 `openspec/changes/archive/*-<name>/` 存在, **THEN** 跳过 (按 archived 处理)

## 技术约束

- `git add improvement-approved.md` 必须在 approve_proposal.sh 退出**前**调用 (fail-fast on git error)
...
- `improvement-approved.md` 格式按 docs/improvement-approved-format.md (Markdown 表格)

## 验收标准

- [ ] approve_proposal.sh 退出时 `git status --porcelain improvement-approved.md` 包含 `M improvement-approved.md`
- [ ] plan 阶段 plan-done gate 加 improvement-approved.md 脏检查 (warning 级别)
