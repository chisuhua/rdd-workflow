...
- **Out Scope**:
  ...
  - 不与 `add-full-regression-gate` 提案的 pre-commit 段重叠 — 它管"build 文件 → ctest"，本提案管"代码路径 ↔ change 联动"

### 与姐妹提案的协同关系
| 提案 | 关系 |
|------|------|
| add-full-regression-gate (P0) | **串联协作** — 同一 pre-commit 文件，openspec-gate 先（cheap ~50ms），regression-gate 后（expensive ~30s） |
| add-config-validation (P0) | **配置联动** — `openspec_gate:` 节由 `config_schema.json` 校验 |
| add-progressive-linting (P2) | **互不干扰** — linting 是独立维度，本提案不触及 |
| 依赖建议 | add-full-regression-gate 优先落地，本提案后接入其 pre-commit 串联点 |
...
- **THEN** 两段按顺序执行：openspec-gate 先（~50ms 检测），regression-gate 后（~30s）。任意失败阻塞 commit
...
- **MUST**:
  ...
  - `SKIP_OPENSPEC_GATE=1` 环境变量可跳过 hook（与 sister proposals 的 `SKIP_*` 约定一致）
  - hook 检测执行时间：检测逻辑 ≤ 50ms，包含 I/O 在内总上限 ≤ 200ms（不显著拖慢 commit）
...
### 自动化
- `tests/integration/test_openspec_gate.bats` PASS，覆盖场景 A / B / C / D / E
...
- `INSTALL.md` 增加 hooks 安装说明
- `skills/openspec-gate/SKILL.md` 描述完整，包含 5 个场景的 GIVEN / WHEN / THEN
