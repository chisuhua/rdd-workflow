...
1. **字段命名**：snake_case（与现有 schema 一致）
2. **时间字段**：统一 `format: date-time`（ISO-8601）
3. **Hub Issue 引用**：统一 pattern `^[a-zA-Z0-9_-]+/[a-zA-Z0-9_-]+#[0-9]+$`
4. **可扩展性**：使用 `additionalProperties: true` 允许后续字段添加
5. **版本字段**：每个 schema 必须有 `version` 字段（const）
6. **跨 schema 引用**：startship 格式（`hub_issue` pattern）统一
