...
3. **实际痛点**：18 个现有改进提案的正文中提及了依赖关系（如 `add-openspec-gate` 的依赖建议 `add-full-regression-gate` 优先落地），但没有结构化字段承载；所有 proposal 在 `improvement-approved.md` 中是平铺列表，无法按 feature 分组规划。
...
- **In Scope**:
  ...
  - `improvement-approved.md` 新增两个可选列：`依赖`、`特性`（缺省为空）
...
- **Out Scope**:
  ...
  - 不修改 `improvement-suggestions.md` 格式
...
- SHOULD `improvement-approved.md` 新增的 `依赖` / `特性` 列缺省显示 `-`
...
- [ ] `improvement-approved.md` 可选显示 `依赖` / `特性` 列（缺省 `-`）
