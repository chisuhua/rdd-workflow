...
- **SKILL.md frontmatter 扩展**：4 个 SKILL.md（guide-arch / guide-design / guide-plan / guide-ship）的 YAML frontmatter 添加 `role:` 顶层字段，含：
...
- **试点实施（PR#1）**：ADR-0028 + guide-arch SKILL.md frontmatter 落地 + 2 个 bats 测试（验证 YAML 解析 + 边界字段存在性）
- **批量实施（PR#2-5）**：guide-design / guide-plan / guide-ship 三个 SKILL.md 各 1 个 PR，每个 PR 1 个 bats 测试
...
|------|------|------|------|------|
| `docs/adr/ADR-0028-role-model-per-phase.md` | ✏️ 新建 | — | — | — |
| `skills/guide-arch/SKILL.md` | ✏️ frontmatter+正文 | — | — | — |
| `skills/guide-design/SKILL.md` | — | ✏️ | — | — |
| `skills/guide-plan/SKILL.md` | — | — | ✏️ | — |
| `skills/guide-ship/SKILL.md` | — | — | — | ✏️ |
| `tests/integration/test_skill_role_*.bats` | ✏️ 新建 | ✏️ 新建 | ✏️ 新建 | ✏️ 新建 |
...
- **frontmatter YAML schema 严格**：新增字段必须 snake_case（如 `human_involvement`），类型与默认值在 `skills/_lib/schemas/skill_role_schema.json` 集中定义
...
- **bats 测试覆盖**：每个 PR 必须有至少 1 个 bats 测试验证 frontmatter 解析 + 必填字段存在性
...
- **PR#1 优先落地**：ADR + guide-arch 试点，剩余 3 个 SKILL.md 待 PR#1 验证后再批量推进（方案 B）
...
- **AC-4（bats 测试覆盖）**：新增 4 个 bats 文件（每个 PR 一个），全部 pass：
  - `tests/integration/test_skill_role_arch.bats`
  - `tests/integration/test_skill_role_design.bats`
  - `tests/integration/test_skill_role_plan.bats`
  - `tests/integration/test_skill_role_ship.bats`
...
- **AC-7（AGENTS.md 同步）**：`rdd-workflow/AGENTS.md` "关键约定"章节包含指向 ADR-0028 的引用块
- **AC-8（PR#1 验证通过）**：ADR-0028 + guide-arch frontmatter + 2 个 bats 测试组成的 PR#1 能独立 ship，剩余 3 个 SKILL.md 按 PR#2-5 顺序推进

### 验证方法

- `./test.sh --full` 跑全量回归（含 4 个新 bats + 现有 117 个集成测试）
- 手工 review 每个 PR 的 frontmatter diff，确认字段值准确
- 模拟场景 1-4（关键场景），用真实对话验证 AI 行为是否符合 frontmatter 声明

### 排除（不在本提案验收范围）

- AI 是否**实际遵循** frontmatter 角色定义（属 LLM 行为，不在本提案验证范围内）
- 其他子技能（propose / execute / status / deps / roadmap / feature / rdd-session / add-improve / rdd-env-check / rdd-doctor）的角色定义（属后续提案范围）