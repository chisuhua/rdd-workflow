...
- **rdd-doctor proposal-table 类别**(已实施): 检测 `improvement-approved.md` 表格行 column count 与预期(4 列)的一致性
- **rdd-workflow improvement-suggestions.md format 文档**(已采纳): 表格格式必须严格 4 列(提案 | 优先级 | 来源 | 添加时间)
- **现状缺陷**: `improvement-approved.md` 存在 column drift(150 处 WARNING),可能是早期限表格格式不严格,后续追加行格式漂移导致 rdd-doctor 无法精确解析
- **类比 anchor**: 与 `fix-doc-truth-sync` (P0 修复 INSTALL.md / package.json skills count 不一致) 同类问题 — 文档/索引 drift

## 范围

- **In Scope**:
 - 检查 `improvement-approved.md` 中所有 4-列格式行,确认 column drift 的真实位置
...
- **Out Scope**:
 ...
 - NOT 修改 `improvement-suggestions.md`(结构正确,刚被 sync-approved-to-suggestions 重构过)
 - NOT 修改 docs/adr/(无相关 ADR 涉及)

## 关键场景

- GIVEN improvement-approved.md 中某行有 3 列(缺优先级/来源/添加时间之一)
...
- `rddf doctor --category proposal-table --quiet` 退出 0(improvement-approved.md 零 drift)
