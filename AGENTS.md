...
`proposal-suggestions.md` 在项目根目录, 随 git 版本控制, 格式为 **Markdown 表格** (索引到 `improvements/*.md`). 审查通过后添加到 `proposal-approved.md` (同为 Markdown 表格格式). 详见 `docs/proposal-suggestions-format.md` 和 `docs/proposal-approved-format.md`.
...
2.5. **Post-archive cleanup hook (post-archive-cleanup-hook)** — 在 archive 主体完成后,自动 idempotent 清扫残留(deleted tracked `.rddf/plans/<name>.md` 等 + modified critical `proposal-approved.md`)。详见 `_lib/post_archive_cleanup.sh` 与 `improvements/post-archive-cleanup-hook.md`。SKIP via `SKIP_POST_ARCHIVE_CLEANUP=yes`; debug via `DRY_RUN_POST_ARCHIVE_CLEANUP=yes`。
...
10. **proposal-suggestions.md 格式为 Markdown 表格** — 索引到 `improvements/*.md`, 用 `list_improvements()` 或 `read_improvement_entries()` 读取, 审查通过后添加到 `proposal-approved.md`
