...
**实现状态**: 此规则已通过 [`add-full-regression-gate`](.rddf/improvements/add-full-regression-gate.md) 提案（`proposal-approved.md` P0, 2026-07-28）正式批准。当前 `rdd-builder`（v4.0+；v2.x~v3.x 时期为 `guide-ship`）的 `archive_gate_check()`（`_lib/archive.sh`）只检查 `tasks.md` 完成度，**未自动化强制回归门**。在 `rdd-builder` 实施该提案之前，本节作为 agent 的硬性行为约束。
