...
|---|---|---|---|---|
| `feat-fix-audit-findings` | 2026-08-26 文档与代码一致性审计后续修复（18 个 audit-followup 提案） | phase-1, phase-2, phase-3, phase-4 | done (4 sub 已 ship 2026-09-10，归入 improvement-approved.md L186-189) | `.rddf/roadmap/features/feat-fix-audit-findings.md` |
| `feat-fix-archive-gaps-v2` | 第二波归档治理改进（ADR 索引自动同步 / CHANGELOG-USAGE 同步 / verifier-archive-gate 边界明确化 / 第 3 波 doc drift 清理，4 个 P2→P1 升级提案） | phase-1, phase-2, phase-3, phase-4 | done (4 sub 全 ship: adr-index-auto-sync 08-28 + changelog-usage-sync 08-28 + verifier-archive-gate-clarification 09-11 + fix-doc-drift-followup-3 09-09) | `.rddf/roadmap/features/feat-fix-archive-gaps-v2.md` |
...
两者 schema 在 `_lib/schemas/` 下（`skills/_lib/` 是向后兼容 shim）, 改 schema 必须 bump version 字段.
`improvement-suggestions.md` 在项目根目录, 随 git 版本控制, 格式为 **Markdown 表格** (索引到 `.rddf/improvements/*.md`). 审查通过后添加到 `improvement-approved.md` (同为 Markdown 表格格式). 详见 `docs/improvement-suggestions-format.md` 和 `docs/improvement-approved-format.md`.
...
**Schema 版本**: v1 (字段定义见 `_lib/schemas/arch_handoff_schema.json`; `skills/_lib/schemas/` 为向后兼容 shim).
...
- 当前最新编号: **ADR-0053** (Layer 0 渐进式上下文注入架构, 2026-09-22; `docs/adr/` 取最大值)
- **架构指针 (P1-2 sync, 2026-09-15)**: 较新的关键 ADR 包括 ADR-0023 (rename spec-workflow→rdd-workflow)、ADR-0024 (deps-driven execution mode)、ADR-0025 (design proposal creation)、ADR-0028 (role model per phase)、ADR-0030 (hub-and-spoke federation)、ADR-0033 (submodule-aware)、ADR-0034 (rdd-verifier 5th phase)、ADR-0035 (verifier-archive-gate 双轨边界)、ADR-0043 (v4 stage-merge)、ADR-0045 (inline ac-verifier)、ADR-0047 (rdd-quick bypass)、ADR-0048 (v4 stage-merge revision)、ADR-0049 (rdd-builder P0 LLM integration)、ADR-0050 (auto-pick mode)、ADR-0051 (skill description convention)。任何 agent 在做架构决策前应至少扫读这三篇。
...
2.5. **Post-archive cleanup hook (post-archive-cleanup-hook)** — 在 archive 主体完成后,自动 idempotent 清扫残留(deleted tracked `.rddf/plans/<name>.md` 等 + modified critical `improvement-approved.md`)。详见 `_lib/post_archive_cleanup.sh` 与 `.rddf/improvements/post-archive-cleanup-hook.md`。SKIP via `SKIP_POST_ARCHIVE_CLEANUP=yes`; debug via `DRY_RUN_POST_ARCHIVE_CLEANUP=yes`。

3. **Phase 3 archive**: 由 `skills/rdd-builder/scripts/ship_archive.sh::archive_change_for_mode` 自动 merge → `openspec archive` → cleanup worktree + branch。
...
### 归档流程 (`rdd-builder` P3，v4.0+；v2.x~v3.x 时期为 `guide-ship` Phase 3)

`rdd-builder` 归档时自动检测模式:
...
- **Skip when `git.openspec_tracked: false`** (per `complete-project-yaml-config-gaps` spec §archive-openspec-tracked-skip-git L246-262):`commit_archive_moves` 自身读 `.rddf/project.yaml` 守卫,false 时直接 SKIPPED,不产生 commit;`archive_change` 的 false 分支也不再触发该 helper。两道闸门互为 defense-in-depth。
- **Idempotent**:已 commit 后再调用,working tree 干净 → 立即 exit 0,无新 commit。
- **Coverage**:在 worktree 模式 (`archive_change`) 和 lightweight 模式 (`rdd-builder.md` P3 inline 调用) 都生效。
...
### Ship 阶段 `rdd-builder/scripts/ship_*.sh` 提取（v2.0.5 新增，v2.0.8 Phase 2 迁至 per-skill scripts/）

`rdd-builder`（v4.0+；v2.x~v3.x 时期为 `guide-ship`）v2.0 起按 Phase 把超过 50 行的内联 bash 块提取为 3 个脚本 (v2.0.8 前在 `_lib/`，Phase 2 重组后迁至 `rdd-builder/scripts/`，v4.0+ 仍保留 `ship_*.sh` 命名以兼容历史测试):

| Script | Source Phase | Public functions |
|--------|-------------|------------------|
| `skills/rdd-builder/scripts/ship_plan.sh` | Phase 1 (plan) | `check_artifacts_committed`, `detect_execution_mode`, `setup_execution_workspace`, `share_submodules_to_worktree`, `generate_implementation_plan`, `record_iteration_status` |
| `skills/rdd-builder/scripts/ship_review.sh` | Phase 2.5 (review) | `handle_review_action` (4-option dispatch) |
| `skills/rdd-builder/scripts/ship_archive.sh` | Phase 3 (archive) | `detect_archive_mode`, `check_feature_integrity`, `archive_change_for_mode` |

`guide-ship.md`（历史前身）由 1361 → 842 行（净减 519 行）。每个 script 都通过 `bats tests/integration/test_ship_*_extraction.bats` 锁定（功能性测试 + 结构性 grep 双保险），遵循 P1-14 archive.sh 提取的同款模式。Phase 2 的 54 行"读取所有 tasks.md 进度"块是后续 P3-3 候选提取目标。
...
完整契约见 `docs/superpowers/specs/2026-08-05-rdd-builder-execution-contract.md`（v4.0+ 路径；v3.0 时期名为 `2026-08-05-guide-ship-execution-contract.md`）。要点:

- `rdd-builder`（v4.0+；v3.0 时期为 `guide-ship`）Phase 1 通过 `rddf discover-ship-changes` 统一发现候选,单 change 自动选择;多 change 显示菜单。
...
| Python function | Source lines | Responsibility |
|------------------|--------------|----------------|
| `set_suggestion_status` | 531-548 | Update improvement-approved.md entry status |
...
`skills/*.md` 提取第二批 — 6 个内联 bash 块横跨 4 个 skill 文件 (~580 行) 迁移到 `_lib/`（v2.0.8 Phase 2 重组后迁至各 skill 的 `scripts/` 目录;**v4.0+ 名称以 `rdd-*` 为准，旧 `guide-*` 名称在 Round 3 硬移除**）:

| Task | Skill (v4 canonical) | Lines | Helper(s) | Tests | 备注 |
|------|---------------------|-------|-----------|-------|------|
| 1 | `rdd-arch` Phase 1 Steps 1-5 | 96 | `rdd-arch/scripts/arch_env_check.sh` | 9 bats | openspec CLI 检查 + 工件发现 |
| 2 | `rdd-arch` handoff writer | 88 | `rdd-arch/scripts/write_arch_handoff.{sh,py,env.py}` | 10 unit + 8 bats | 3 文件 env-var pattern |
| 3 | `rdd-planner`（原 `guide-plan`）Phase 0 intake | 79 | `rdd-planner/scripts/plan_intake.sh` | 7 bats | Oracle C1 fix：去 bash string interp |
| 4 | `rdd-planner`（原 `guide-plan`）plan-done gate + handoff | 150 | `rdd-planner/scripts/plan_done_gate.{sh,py,env.py}` | 8 unit + 9 bats | 双闸门 + handoff 写入 |
| 5 | `rdd-builder`（原 `guide-ship`）P2 monitor | 54 | `rdd-builder/scripts/ship_monitor.sh` | 7 bats | P3-3 maintainer-flagged target |
| 6 | `execute` worktree auto-detect | 113 | `execute/scripts/select_worktree.sh` | 8 bats | 大块，type-1 helper |
...
| Task | Skill (v4 canonical) | Lines | Helper(s) | Tests | 备注 |
|------|---------------------|-------|-----------|-------|------|
| B1 | `rdd-arch` gap analysis | 85 | `rdd-arch/scripts/arch_gap_analysis.sh` | 8 bats | generator + viewer |
| B2 | `rdd-arch` Phase 5 dual gate | 38 | `rdd-arch/scripts/arch_done_gate.sh` | 6 bats | ADR + roadmap gate |
| B3 | `rdd-planner`（原 `guide-plan`）deps-candidates | 38 | `rdd-planner/scripts/plan_deps_candidates.{sh,py,env.py}` | 6 unit + 6 bats | **SECURITY FIX**: oracle C1 |
| B4 | `rdd-planner`（原 `guide-plan`）queue overview | 50 | `rdd-planner/scripts/plan_queue_overview.sh` | 6 bats | 5-state summary |
| B5 | `rdd-planner`（原 `guide-plan`）feature progress | 34 | `rdd-planner/scripts/plan_feature_progress.sh` | 5 bats | per-feature |
| B6 | `status` render_status Mode A | 45 | `status/scripts/status_render_mode_a.sh` | 6 bats | iteration + fallback |
| B7 | `execute` tasks writeback | 34 | `execute/scripts/tasks_writeback.sh` | 6 bats | **BUG FIX**: sub() regex issue |
| B8 | `execute` roadmap progress | 50 | `execute/scripts/update_roadmap_progress.{sh,py,env.py}` | 7 unit + 6 bats | **SECURITY FIX**: oracle C1 |
| B9 | `execute` Step 7 report | 88 | `execute/scripts/execute_step7.{sh,py,env.py}` | 6 unit + 8 bats | final report + sync |
| B10 | `rdd-arch` arch-quality-report | 32 | `rdd-arch/scripts/arch_quality_report.sh` | 4 bats | thin wrapper |
...
4-phase environment orchestrator (detect → diagnose → suggest → guided-fix) that fills the orchestration gap left after Layer 0→3 progressive context architecture (ADR-0052).
...
- **CLI 子命令注册**: `_lib/cli/__init__.py::_ROUTES` 添加 `"env-bootstrap": "skills._lib.cli.env_bootstrap_cmd:cmd_env_bootstrap"`
- **`_NO_STATE_CHECK` 白名单**: `_lib/cli/__main__.py` 把 `env-bootstrap` 加入 `{"setup", "doctor", "env-bootstrap"}` (在非 rdd-workflow 项目目录友好退出 exit 3)
- **修复白名单**: 仅 `ai-context-bootstrap: 未部署` + `ai-context-bootstrap: 块已陈旧` 可自动修复 (per AC-6)
- **退出码语义**: 0 健康 / 1 警告已自动修 / 2 关键失败 / 3 环境错误 (对齐 openspec validate)
- **报告 schema**: `_lib/schemas/env_bootstrap_report_schema.json` version=1, 8 个 required top-level fields
- **ADR 索引**: 见 `docs/adr/ADR-0053-rdd-env-bootstrap-orchestrator.md`
- **与 rdd-quick 边界**: rdd-env-bootstrap 是 user-facing 编排 (有完整 openspec change + worktree 流程), rdd-quick 是小改动旁路 (per ADR-0047, 完全跳过 change) — 不可混淆

### 与既有 skill 的边界

| Skill | 与 rdd-env-bootstrap 关系 |
|---|---|
| `rdd-env-check` | phase 内嵌快速检查 (≤200ms 缓存); env-bootstrap 是用户主动全流程 (≤10s 含 fix) |
| `rdd-doctor` | 只读多 category 诊断; env-bootstrap 调 doctor 但可引导式修复 (doctor 不动) |
| `rdd-hub-bootstrap` | Hub repo (rdd-hub) 引导式初始化; env-bootstrap 是任意第三方项目 |
| `rdd-quick` | 小改动旁路 (no openspec change, per ADR-0047); env-bootstrap 走完整 P0-P3 |
...
| `git.openspec_tracked: false` | `_lib/archive.sh` L546 分支 + `commit_archive_moves` L808 守卫 | 仍走 `openspec archive` 但不产生 git commit；rdd-quick 不调 archive |
...
10. **improvement-suggestions.md 格式为 Markdown 表格** — 索引到 `.rddf/improvements/*.md`, 用 `list_improvements()` 或 `read_improvement_entries()` 读取, 审查通过后添加到 `improvement-approved.md`
