...
| [fix-cmd-help-handling](.rddf/improvements/fix-cmd-help-handling.md) | P1 | 2026-09-25 rdd-verifier re-verify audit of rdd-workflow-e2e PR #1 (CI regression) — 36 个 subcommand 中 2 个 `rddf <sub> --help` 违反 argparse 契约:`rddf contract-check --help` 返回 EXIT 2 (自定义 `action="store_true"` 覆盖 argparse 自动 help);`rddf archive-sync --help` 把 `--help` 当 change name 查找 iteration.json 返回 EXIT 1。CI 在 rdd-workflow-e2e `test_rddf_cli_all_subcommands.bats::AC-6` fail,`报告新增失败: 1` | 2026-09-25 | 待审查 |
