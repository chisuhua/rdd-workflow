...
@test "update_roadmap_progress_helper_exists" {
  [ -f "$REPO_ROOT/skills/execute/scripts/update_roadmap_progress.sh" ]
  [ -f "$REPO_ROOT/skills/execute/scripts/update_roadmap_progress.py" ]
  [ -f "$REPO_ROOT/skills/execute/scripts/update_roadmap_progress_env.py" ]
  bash -c "cd '$REPO_ROOT' && source skills/execute/scripts/update_roadmap_progress.sh && declare -f update_roadmap_progress" | grep -q 'update_roadmap_progress'
}
...
@test "execute_invokes_helper" {
  grep -q 'source.*scripts/update_roadmap_progress.sh' "$REPO_ROOT/skills/execute/SKILL.md"
...
@test "oracle_c1_no_bash_string_interpolation" {
  ...
  ! grep -n "python3.*'\$" "$REPO_ROOT/skills/execute/scripts/update_roadmap_progress.sh"
}

@test "no_in_place_sed_edit_for_roadmap_meta" {
  # The helper must NOT use 'sed' for roadmap-meta (Oracle C1 unsafe pattern)
  ! grep -q "sed.*roadmap-meta" "$REPO_ROOT/skills/execute/scripts/update_roadmap_progress.sh"
}