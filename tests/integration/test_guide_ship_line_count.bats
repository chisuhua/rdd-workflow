...
@test "guide-ship.md is ≤ 850 lines after extraction (was 1361)" {
  local line_count
  line_count=$(wc -l < "$REPO_ROOT/skills/guide-ship/SKILL.md")
  [ "$line_count" -le 850 ]
}
...
@test "guide-ship.md wires all 3 extracted helpers" {
  ...
  grep -q 'ship_plan.sh' "$REPO_ROOT/skills/guide-ship/SKILL.md"
  grep -q 'ship_review.sh' "$REPO_ROOT/skills/guide-ship/SKILL.md"
  grep -q 'ship_archive.sh' "$REPO_ROOT/skills/guide-ship/SKILL.md"
}
