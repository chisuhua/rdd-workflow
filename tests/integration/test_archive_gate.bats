...
@test "archive-gate: blocks change with 0 completed tasks" {
    ...
    run bash -c "source '$PROJECT_ROOT/skills/_lib/archive.sh' && cd '$TMP' && archive_gate_check 'test-zero'"
    [ "$status" -eq 1 ]
    [[ "$output" =~ "未实现" ]]
}
...
@test "archive-gate: passes change with completed tasks" {
    ...
    run bash -c "source '$PROJECT_ROOT/skills/_lib/archive.sh' && cd '$TMP' && archive_gate_check 'test-done'"
    [ "$status" -eq 0 ]
}
...
@test "archive-gate: skips with FORCE_ARCHIVE_INCOMPLETE" {
    ...
    FORCE_ARCHIVE_INCOMPLETE=yes \
        run bash -c "source '$PROJECT_ROOT/skills/_lib/archive.sh' && cd '$TMP' && archive_gate_check 'test-force'"
    [ "$status" -eq 0 ]
}
...
@test "archive-gate: blocks when tasks.md is missing (no fail-open)" {
    ...
    source "$PROJECT_ROOT/skills/_lib/archive.sh"
    run bash -c "source '$PROJECT_ROOT/skills/_lib/archive.sh' && cd '$TMP' && archive_gate_check 'test-missing'"
    [ "$status" -eq 1 ]
    [[ "$output" =~ "缺失" ]]
}
...
@test "archive-gate: reads tasks.md from explicit tasks_root (worktree path)" {
    ...
    source "$PROJECT_ROOT/skills/_lib/archive.sh"
    run bash -c "source '$PROJECT_ROOT/skills/_lib/archive.sh' && archive_gate_check 'worktree-change' '$WT'"
    [ "$status" -eq 0 ]
}