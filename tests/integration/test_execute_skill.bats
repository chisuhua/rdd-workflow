...
@test "execute_skill uses main_repo_root for PROJECT_ROOT" {
  ...
  grep -q 'main_repo_root' "$f" || grep -q 'main_repo_root' "$REPO_ROOT/skills/execute/scripts/select_worktree.sh"
}
...
@test "execute_skill honors EXECUTE_CHOICE escape hatch" {
  ...
  grep -q 'EXECUTE_CHOICE' "$f" || grep -q 'EXECUTE_CHOICE' "$REPO_ROOT/skills/execute/scripts/select_worktree.sh"
