...
@test "phase 4: name-pattern 只触发于 SKELETON_MODE=false 时" {
    grep -qi "SKELETON_MODE.*false" "$BATS_TEST_DIRNAME/../../skills/propose/SKILL.md" || return 1
    return 0
}

@test "phase 4: 匹配后设置 SKELETON_MODE=true" {
    grep -qi "SKELETON_MODE.*true" "$BATS_TEST_DIRNAME/../../skills/propose/SKILL.md" || return 1
    return 0
}
...
@test "frontmatter: propose.md 仍合法" {
    python3 -c "
import yaml
with open('skills/propose/SKILL.md') as f:
    content = f.read()
