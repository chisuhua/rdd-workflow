...
# Round B extraction: rdd-arch.md L564-L595 arch-quality-report invoker (~32 lines)
...
#   2. rdd-arch.md inline block markers removed.
#   3. rdd-arch.md sources and invokes helper.
#   4. Helper writes JSON report with "passed" field when successful.

load ../test_helper

@test "arch_quality_report_helper_exists" {
  [ -f "$REPO_ROOT/skills/rdd-arch/scripts/arch_quality_report.sh" ]
  bash -c "cd '$REPO_ROOT' && source skills/rdd-arch/scripts/arch_quality_report.sh && declare -f run_arch_quality_report" | grep -q 'run_arch_quality_report'
}

@test "guide_arch_inline_quality_report_removed" {
  ! grep -q 'arch_quality_gate import ArchQualityReport' "$REPO_ROOT/skills/rdd-arch/SKILL.md"
}

@test "guide_arch_invokes_quality_report_helper" {
  grep -q 'source.*scripts/arch_quality_report.sh' "$REPO_ROOT/skills/rdd-arch/SKILL.md"
  grep -q 'run_arch_quality_report' "$REPO_ROOT/skills/rdd-arch/SKILL.md"
}
...
PYEOF
  ...
  bash -c "cd '$tmpdir' && PROJECT_ROOT='$tmpdir' bash -c \"source '$REPO_ROOT/skills/rdd-arch/scripts/arch_quality_report.sh' && run_arch_quality_report\"" 2>&1 || true
