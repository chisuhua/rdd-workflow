...
# After post_archive_cleanup runs, improvement-approved.md should reflect
...
# scan so improvement-approved.md stays consistent with the archive/ tree.
#
...
# helper: build a improvement-approved.md with one approved entry
...
seed_approved_proposal() {
  ...
  # improvement-approved.md: entry sits in ## 已批准提案 section
  cat > "$PROJECT_ROOT/improvement-approved.md" <<EOF
## 已批准提案
...
|------|--------|----------|------|
EOF
  git add improvement-approved.md openspec/changes/archive/2026-08-21-${name}
  git commit -q -m "seed"
}

@test "post_archive_cleanup: sync improvement-approved.md — archived entry moves from approved to implemented" {
  seed_approved_proposal "fix-ship-orphan" "P1"

  # Pre-condition: entry in ## 已批准提案 section
  run python3 -c "
content = open('$PROJECT_ROOT/improvement-approved.md').read()
...
@test "post_archive_cleanup: idempotent — second run on same archive dir produces no change" {
  ...
  content_after_first=$(cat "$PROJECT_ROOT/improvement-approved.md")
  ...
  content_after_second=$(cat "$PROJECT_ROOT/improvement-approved.md")

  [ "$content_after_first" = "$content_after_second" ]

  # Verify exactly one occurrence in the file
  local count
  count=$(grep -c "fix-double-archive" "$PROJECT_ROOT/improvement-approved.md" || true)
  [ "$count" -eq 1 ]
}
...
with open('$PROJECT_ROOT/improvement-approved.md', 'w') as f:
    f.write(new_content)
print('OK')
"
