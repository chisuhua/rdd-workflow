...
#
# Locks the extraction of guide-arch.md inline case handlers (L126/L201/L333/L458)
# to skills/guide-arch/scripts/arch_roadmap_menu.sh::handle_arch_menu().
#
# Tests:
#   1. Helper script exists with handle_arch_menu function.
#   2. guide-arch.md no longer inlines the original case block markers.
#   3. guide-arch.md sources and invokes helper at each phase.
...
@test "arch_roadmap_menu_helper_exists" {
  [ -f "$REPO_ROOT/skills/guide-arch/scripts/arch_roadmap_menu.sh" ]
  bash -c "cd '$REPO_ROOT' && source skills/guide-arch/scripts/arch_roadmap_menu.sh && declare -f handle_arch_menu" | grep -q 'handle_arch_menu'
}
...
@test "guide_arch_inline_menu_block_removed" {
  ...
  count=$(grep -cF 'r|refresh) continue ;;  # 重新展示菜单' "$REPO_ROOT/skills/guide-arch/SKILL.md" || true)
  [ "$count" -eq 0 ]
}

@test "guide_arch_invokes_menu_helper_at_each_phase" {
  # 4 phases (setup, adr-create, architecture, roadmap-define) all source + call.
  count=$(grep -c 'scripts/arch_roadmap_menu.sh' "$REPO_ROOT/skills/guide-arch/SKILL.md")
  [ "$count" -ge 4 ]
  count=$(grep -c 'handle_arch_menu' "$REPO_ROOT/skills/guide-arch/SKILL.md")
  [ "$count" -ge 4 ]
}

@test "handle_arch_menu_exits_on_quit" {
  run bash -c "cd '$REPO_ROOT' && source skills/guide-arch/scripts/arch_roadmap_menu.sh && handle_arch_menu 'q'"
  [ "$status" -eq 0 ]
}

@test "handle_arch_menu_exits_on_exit" {
  run bash -c "cd '$REPO_ROOT' && source skills/guide-arch/scripts/arch_roadmap_menu.sh && handle_arch_menu 'exit'"
  [ "$status" -eq 0 ]
}

@test "handle_arch_menu_returns_2_on_refresh" {
  run bash -c "cd '$REPO_ROOT' && source skills/guide-arch/scripts/arch_roadmap_menu.sh && handle_arch_menu 'r'; rc=\$?; echo \"rc=\$rc\""
  [ "$status" -eq 0 ]
  echo "$output" | grep -q 'rc=2'
}

@test "handle_arch_menu_prints_help_on_question_mark" {
  run bash -c "cd '$REPO_ROOT' && source skills/guide-arch/scripts/arch_roadmap_menu.sh && handle_arch_menu '?'"
  [ "$status" -eq 0 ]
  echo "$output" | grep -q '可用命令'
}

@test "handle_arch_menu_prints_invalid_on_unknown_input" {
  run bash -c "cd '$REPO_ROOT' && source skills/guide-arch/scripts/arch_roadmap_menu.sh && handle_arch_menu 'zzz'"
  [ "$status" -eq 0 ]
