...
@test "feature_summary: helper file exists with render_feature_summary function" {
  [ -f "$REPO_ROOT/skills/feature/scripts/feature_summary.sh" ]
  bash -c "source '$REPO_ROOT/skills/feature/scripts/feature_summary.sh' && declare -f render_feature_summary" | grep -q 'render_feature_summary'
}
...
@test "feature_summary: feature.md sources and calls helper" {
  grep -q 'source.*scripts/feature_summary.sh' "$REPO_ROOT/skills/feature/SKILL.md"
...
@test "feature_summary: handles missing iteration.json gracefully" {
  local tmpdir
  tmpdir=$(mktemp -d)
  output=$(PYTHONPATH="$REPO_ROOT" PROJECT_ROOT="$tmpdir" bash -c "source '$REPO_ROOT/skills/feature/scripts/feature_summary.sh' && render_feature_summary" 2>&1 || true)
  rm -rf "$tmpdir"
