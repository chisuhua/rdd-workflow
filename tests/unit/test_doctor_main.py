...
def test_aggregate_runs_all_7_categories(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """aggregate_findings invokes all 7 checker modules and combines results."""
    ...
    assert set(categories_checked) == {
        ...
        "tasks-checkbox", "migration-residue", "orphan-gates",
    }
...
def test_checkers_dict_has_7_entries():
    """Lock the public contract: exactly 7 categories wired (6 + orphan-gates)."""
    assert len(_CHECKERS) == 7