...
install_global_symlinks() {
    ...
    echo "    skill_use(\"rdd-arch\")     # Stage 1: ADR + arch docs (slim v3)"
    echo "    skill_use(\"rdd-planner\")  # Stage 2: roadmap + proposal authoring"
    echo "    skill_use(\"rdd-builder\")  # Stage 3: 6-phase state machine"
    echo "    skill_use(\"rdd-verifier\") # Stage 4: batch AC verification"
    echo ""
    echo "  CLI 工具:"
    echo "    rddf status         # 查看工作流状态"
    echo "    rddf sessions       # 管理 session"
    echo "    rddf rdd-verify     # 批量验证 ship-done changes"
    echo ""
    echo "  如果需要重新加载:"
    echo "    bash $0 --global"
    echo ""
