...
def _detect_cycle_message(graph: Dict[str, List[str]]) -> Optional[str]:
    try:
        from _lib.cross_repo_deps import detect_cycle
    except ImportError:
        return None
...
def check_cross_repo_deps_blocked(
    ...
    """
    from _lib.cross_repo_deps_cache import (
    ...
    try:
        from _lib.cross_repo_deps import build_cross_repo_graph
...
def _gather_local_spokes(project_root: PathLike) -> Dict[str, List[Dict[str, str]]]:
    from _lib.cross_repo_deps import parse_spoke_iteration
