...
- [x] Create `skills/cross-repo-protocol/` directory structure
...
- [x] Create `skills/cross-repo-protocol/mcp_client.py` — MCP Client library
...
- [x] Implement `hub_read_issue` tool in `MCPClient`
...
- [x] Implement `hub_create_issue` tool in `MCPClient`
...
- [x] Implement `hub_update_status` tool in `MCPClient`
...
- [x] Implement `hub_sync_contract` tool in `MCPClient`
...
- [x] Create `.rddf/state/.mcp-trace.jsonl` trace logging
...
- [x] Integrate trace logging into `MCPClient.call_tool()`
...
- [x] Implement `MCPClient._fallback_to_rest()` method
...
- [x] Implement `GITHUB_TOKEN` environment variable reading
  - Read token in `MCPClient.__init__()`
  - Pass to MCP Server via SDK auth mechanism
  - Pass to REST fallback via `requests` headers

- [x] Implement `MCP_TRANSPORT` environment variable for dual transport
  - Default to `stdio`
...
- [x] Implement rate limit detection and retry
...
- [x] Modify `install.sh` to add `--spoke-init` subcommand
...
- [x] Create `skills/templates/.cursorrules.cross-repo-hub` template file
...
- [x] Create `skills/cross-repo-protocol/SKILL.md` — MCP protocol human-readable documentation
...
- [x] Create unit tests for `mcp_client.py`
...
- [x] Create unit tests for REST fallback
...
- [x] Run `openspec validate add-mcp-cross-repo-protocol` — must pass all gates
