...
## Purpose
This specification defines the installation and prompt integration for the MCP cross-repo protocol. It covers the `install.sh --spoke-init` subcommand that injects `.cursorrules.cross-repo-hub` into Spoke repositories, and the prompt boundary between Spoke AI and Hub AI. Grounded in proposal Section "场景 3：System Prompt 注入".
