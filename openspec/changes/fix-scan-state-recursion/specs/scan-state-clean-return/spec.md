...
STATUS: PROPOSED
DATE: 2026-07-15
CHANGE: fix-scan-state-recursion
RELATED_INCIDENT: 2026-07-15 add-archive-auto-commit ship → clean state → `guide` scanner hung >120s
---

# Capability: scan-state-clean-return

> `skills/_lib/scan-state.sh::check_stale_workflow_state` MUST terminate
> after the optional warning emission, not recurse. Currently ships
> with a self-call on line 220 that hangs the scanner in clean state.
>
