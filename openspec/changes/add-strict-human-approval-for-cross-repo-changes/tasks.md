...
- [ ] **Task 1.1**: Modify `skills/guide-design/scripts/approve_proposal.sh` to detect `**分类**: cross-repo-federation` proposals by reading `roadmap-meta.yaml.category`
...
- [ ] **Task 1.2**: Add `RDDF_REQUIRE_HUB_APPROVAL` environment variable handling
...
- [ ] **Task 1.3**: Implement interactive stdin prompt for `--manual` mode
...
- [ ] **Task 2.1**: Create `skills/_lib/cross_repo_audit.py` for audit log management
...
- [ ] **Task 2.2**: Implement Hub Issue status recheck before approval
...
- [ ] **Task 2.3**: Add Hub Issue validation with `--hub-issue` argument
...
- [ ] **Task 3.1**: Modify `skills/guide-design/scripts/design_content_review.sh` for cross-repo category
...
- [ ] **Task 3.2**: Ensure `STRICT_DESIGN_GATE=yes` blocks design-done for unapproved cross-repo proposals
...
- [ ] All 9 acceptance criteria from proposal.md are satisfied
- [ ] `openspec validate add-strict-human-approval-for-cross-repo-changes` passes with no errors
- [ ] No Spoke-side bypass paths exist (verified by code review)
- [ ] Unit tests cover all 5 key paths (auto-block, gate-detect, manual-confirm, audit-write, hub-state-recheck)
