# add-strict-human-approval-for-cross-repo-changes: Specifications

## ADDED Requirements

### Requirement: Cross-repo proposals MUST be blocked from AI auto-approval

When `approve_proposal.sh` is invoked with `--auto-accept` flag or in a context where AI can trigger automatic execution, and the proposal's `**分类**` field contains `cross-repo-federation`, the script MUST hard-block with exit code 3. No AI agent, regardless of privilege level or configuration, may automatically approve a cross-repo proposal.

**Rationale**: ADR-0031 §Decision establishes that cross-repo RFCs MUST have human decision. This requirement enforces that principle at the first point of contact.

#### Scenario: AI attempts auto-approval of cross-repo proposal
**WHEN** an AI agent executes `rddf approve-proposal cross-repo-auth-v2 --auto-accept` on a Spoke repository with a proposal named `cross-repo-auth-v2` that has `**分类**: cross-repo-federation`

**THEN** the command MUST fail with exit code 3

**AND** the output MUST display: `⛔ [STRICT] 提案 'cross-repo-auth-v2' 标记为 cross-repo-federation，AI 自动批准被硬阻断。`

**AND** the output MUST state that human approval via `--manual` is required after Hub Issue is Approved

#### Scenario: Non-cross-repo proposals unaffected
**WHEN** a proposal with `**分类**: feature` (not cross-repo-federation) exists

**THEN** `rddf approve-proposal <name> --auto-accept` proceeds without the cross-repo gate
