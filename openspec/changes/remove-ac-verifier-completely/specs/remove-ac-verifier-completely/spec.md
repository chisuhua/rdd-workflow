...
### Requirement: ac-verifier skill (deprecated per ADR-0045)
**Reason**: v2.0 (ADR-0045) makes this skill obsolete. `rdd-verifier` v2.0+ self-contains LLM verification (the executing AI agent IS the LLM, no external provider config required).

**Migration**: Use `rdd-verifier` v2.0+ for all AC verification workflows.

### Requirement: ac-verify CLI subcommand (deprecated per ADR-0045)
**Reason**: v2.0 (ADR-0045) removes the CLI subcommand. A friendly-error stub remains so users see a clear migration hint instead of a silent failure or `command not found`.

**Migration**: Use `rddf rdd-verify` for all AC verification CLI invocations.