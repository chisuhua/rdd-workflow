...
- **幂等性**：`rddf sync-hub` 必须幂等（同一文件重复拉取不应产生 diff）
- **离线模式**：Hub 网络不可达时，应回退到本地缓存 + 警告，不阻断流程
- **权限最小**：只读 GitHub token 即可运行 `sync-hub` / `watch-hub`；`report-issue` 需要写权限
- **GitHub API 速率限制**：遵守 5000 req/hour 限制；批量操作使用 GraphQL
