...
- **Add** `skills/_lib/config.py` + `defaults.py` — config parser for `.rdd-workflow.json` and `loop.yaml` with priority-merge rules
...
- `configuration`: Multi-source config parser with priority-merge (runtime > loop.yaml > .rdd-workflow.json > env > defaults)
