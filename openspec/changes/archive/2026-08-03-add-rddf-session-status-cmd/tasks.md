...
- [x] 2.1 Write failing test (单元/集成测试, 按 improvements/add-rddf-session-status-cmd.md 关键场景)
- [x] 2.2 Verify test fails (red)
- [x] 2.3 Implement change (按 improvements/add-rddf-session-status-cmd.md 范围 + 技术约束)
- [x] 2.4 Verify test passes (green)
- [x] 2.5 Refactor + commit

## 3. Verification

- [x] 3.1 Run `openspec validate add-rddf-session-status-cmd` (passes)
- [x] 3.2 Run `pytest tests/unit/test_rddf_*.py` (passes)
- [x] 3.3 Run `bats tests/integration/test_*.bats` (passes)
- [x] 3.4 Run `git show HEAD:openspec/changes/add-rddf-session-status-cmd/design.md` (artifact committed)
- [x] 3.5 Run `git show HEAD:openspec/changes/add-rddf-session-status-cmd/tasks.md` (artifact committed)
- [x] 3.6 Run `git show HEAD:openspec/changes/add-rddf-session-status-cmd/.openspec.yaml` (metadata committed)
