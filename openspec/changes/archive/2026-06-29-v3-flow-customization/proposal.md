...
ADR-0012 builds on ADR-0011's step pipeline to expose a **flow customization layer** to users. Today, every rdd-workflow project follows the same default phase step sequence. Real-world projects need to:
...
ADR-0012 introduces `.rdd-workflow/flow.yaml` using an **incremental override** model: users declare only deltas (insert / replace / insert_before) and the default template is merged automatically — so rdd-workflow upgrades that add new default steps do not break user customizations.
...
- **`.rdd-workflow/flow.yaml` schema**: Versioned configuration file with `customizations.<phase>` containing `insert_after` / `insert_before` / `replace` directives
...
- **Backward compatibility**: Without `.rdd-workflow/flow.yaml`, the effective step sequence equals the default template — zero behavior change
