...
STATUS: PROPOSED
DATE: 2026-07-11
CHANGE: add-spec-validation-gates
RELATED: gate-mechanism (Check API for severity-segregated gating)
RELATED: three-phase-skills (skill entry points where validators wire in)
RELATED_INCIDENT: TaskRunner g-gpu-client-default-stub-init v1 (baseline fabrication, 2026-07-11)
RELATED_INCIDENT: TaskRunner g-gpu-client-meyers-singleton-fallback v2 (MODIFIED-on-empty spec, 2026-07-11)
---

# Capability: spec-validation-gates

> Pre-flight validation utilities that catch spec defects (fabricated baseline
> claims and invalid MODIFIED/RENAMED delta targets) BEFORE they propagate to
> implementation or trigger archive abort. Wired into the propose, plan-done,
> and archive entry points of the OpenSpec workflow.
>
> **Status**: Spec-only (PROPOSED). Awaiting implementation.
