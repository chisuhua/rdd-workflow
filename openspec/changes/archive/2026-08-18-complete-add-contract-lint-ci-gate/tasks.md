...
- [x] 1.1 创建 `_lib/cli/contract_check_cmd.py`(参照 `_lib/cli/ac_verify_cmd.py` 模板)
...
- [x] 1.2 在 `_lib/cli/__init__.py` 的 `_ROUTES` 字典中插入 `"contract-check"` 路由
  - 位置: 字典按字母序插入,在 `"deps"` 之后
  - 值: `"contract-check": "skills._lib.cli.contract_check_cmd:cmd_contract_check"`
  - 验证: `list_commands()` 自动包含新条目
- [x] 1.3 在 `skills/guide-plan/scripts/plan_done_gate.sh` 实现 `STRICT_CONTRACT_GATE` 接线
...
- [x] 1.4 README.md §跨项目协同 章节末尾新增 `### CI 集成示例` 子节
...
- [x] 1.5 新增 `tests/integration/test_rddf_contract_check_cli.bats`(≥3 用例)
...
- [x] 1.6 新增 `tests/integration/test_strict_contract_gate_wiring.bats`(≥3 用例)
...
- [x] 1.7 验证现有 contract-check 测试保持 pass(无 regression)
