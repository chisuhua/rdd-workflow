...
**S6 — tasks.md checkbox 与 openspec status 不一致**
- GIVEN `openspec/changes/bar/tasks.md` 标记 5/5 完成，但 `openspec status --json` 报告该 change 仍是 `in-progress`（反之亦然）
- WHEN 跑 doctor
- THEN WARNING，输出 "tasks.md says complete but openspec status says in-progress"；退出码 1
...
新增 1 个 skill + 5 类只读诊断能力：

- **`rdd-doctor` skill**（用户手动调用）：
  - **state-schema-check**：校验 `.rddf/state/{state_vector,sessions,iteration,deps-analysis}.json` 对 `_lib/schemas/` 下对应 schema，捕获 v2 schema 升级后未迁移的旧文件
  - **plan-tdd-check**：校验 `.rddf/plans/*.md` 含 TDD 5 步结构（Write failing test / Verify fail / Implement / Verify pass / Commit），松匹配仅检查 5 个 step 标记存在
  - **roadmap-meta-check**：校验 `openspec/changes/*/roadmap-meta.yaml` 字段齐全 + `manual_deps` / `manual_blocks` 类型正确（ADR-0022）
  - **proposal-table-check**：校验 `proposal-suggestions.md` / `proposal-approved.md` Markdown 表格列数 + 必填字段 + 链接有效性
  - **tasks-checkbox-check**：校验 `openspec/changes/*/tasks.md` checkbox 计数非 0 + 文件存在性（**v1 不交叉 openspec status**：CLI v1.4.1 `status --change X --json` 对缺 `schema` 字段的 change 报 "Invalid metadata"；CLI 缺失/失败时降级为 checkbox-only）
- **双模式输出**：
  - 默认模式：人类可读分级报告（CRITICAL / WARNING / INFO 段），退出码 0/1/2/3
  - `--json` 模式：写 `.rddf/state/.doctor-report.json`（gitignored）
- **可选 flags**：`--category <name>`（5 选 1 过滤）/ `--quiet`（仅最严重级别）
- **退出码分级**（对齐 `openspec validate`）：0 全 OK / 1 仅 INFO+WARNING / 2 有 CRITICAL / 3 checker 内部异常

## Impact

- **Affected specs**: 无（不修改现有 capability specs）
- **Affected code**:
  - 新增 `skills/rdd-doctor/`（SKILL.md + scripts/doctor.sh + scripts/doctor_render.py + scripts/checks/*.py）
  - `tests/integration/test_rdd_doctor.bats`（新增 ≥15 场景）
  - `tests/unit/test_doctor_render.py`（新增 ≥6 场景）
  - `tests/smoke.bats`（新增 rdd-doctor 注册行）
- **Affected docs**:
  - `AGENTS.md`（新增 ~15 行「rdd-doctor」段）
  - `tests/README.md`（新增 1 行入口引用）
- **Affected state files**:
  - `.rddf/state/.doctor-report.json`（gitignored，--json 模式新增）
- **Breaking changes**: 无（纯新增）
- **Compatibility**: doctor 不修改任何现有文件；不修改 4 个 guide SKILL.md / 现有 schema / rdd-env-check；与 ADR-0018/0019（gate 系统）正交
- **Performance budget**: 完整 5 类检查 < 3s；`--category` 单类 < 1s；`--json` 额外开销 < 0.5s（**建议** SHOULD，非硬 AC）
- **Risk level**: 低（只读 + 仅手动触发 + 与现有基础设施无重叠）
