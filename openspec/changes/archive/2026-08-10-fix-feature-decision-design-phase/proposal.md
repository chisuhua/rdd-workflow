...
- `approve_proposal.sh` only reads `PARENT_FEATURE` env var (no header parse)
- `propose_change.py::create_skeleton_change` only accepts `parent_feature` as function param (no header parse)

Users writing `**特性**: wave-core` in `improvements/<name>.md` saw no effect; `feature` view did not recognize it. This change closes the gap.

## What Changes

**In Scope**:
- `skills/guide-design/scripts/approve_proposal.sh`: parse `**特性**` header → `HEAD_FEATURE` → `PARENT_FEATURE` fallback (env var wins)
- `skills/propose/scripts/propose_change.py::create_skeleton_change`: when `parent_feature=None`, parse `**特性**` from `improvements/<name>.md` (param wins)
- Python regex uses `[ \t]*` (not `\s*`) to avoid cross-line latent bug
- New tests: `tests/integration/test_approve_proposal_parent_feature.bats` (3 cases) + `tests/unit/test_propose_change_parent_feature.py` (3 cases)

**Out of Scope**:
- `**类型**` / `**阶段**` / `**分类**` parsing `\s*` cross-line latent bug (separate; not in this change)
- `iteration.json` write path missing `parent_feature` field (separate bug; pre-existing)
- `guide-design` phase feature preview UX (B-scheme; post-stable eval)

## Impact

- **Affected skills**: `guide-design` (approve_proposal.sh) + `propose` (propose_change.py)
- **Breaking change**: No — fallback behavior, explicit overrides (env var / param) unchanged
- **Test coverage**: 3 bats + 3 pytest = 6 new tests lock fallback precedence contract
