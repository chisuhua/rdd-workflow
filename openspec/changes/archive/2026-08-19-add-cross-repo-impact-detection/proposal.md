...
ADR-0030/0032 确立 Hub 联邦架构后，开发者经常在提案后期才意识到需要发起 Hub RFC。当前 `add-improve` 仅生成 head 字段（阶段/分类/类型/特性），不分析提案是否触碰 Hub `contracts/*.yaml`，导致：重复劳动（提案被 approve 后才发现要补 RFC）；跨仓契约变更遗漏；stakeholders 列表需手工整理。

参考 e2e 测试 `tests/integration/test_cross_repo_e2e_real.bats`（13 cases）于 2026-08-19 验证基础设施已可用。
...
- 新增 bats test `tests/integration/test_cross_repo_impact_detection.bats`：覆盖 4 个场景
...
新增 capability（合并到 `openspec/specs/cross-repo-federation/spec.md`）：

- `cross-repo-impact-detection`：提案生成阶段自动识别 Hub 契约影响
...
- AC-1: `add-improve <name>` 在提案正文含 `auth/` 时输出 `⚠️ 检测到 Hub 契约: contracts/auth-v2.yaml`
...
依赖 P0 `fix-federation-gh-cli-integration`（已 archive, 2026-08-19）。
