# rddf-session-auto-archive — Capability Spec

> **Type**: Additive extension to ADR-0017 rddf-session hygiene.
> **Scope**: `skills/rddf-session/scripts/rddf_session_hooks.sh` entry/close hooks.

## Purpose

Automatically invoke `archive_history` when rddf-session hooks run, so
`sessions.json` does not grow unbounded between manual cleanups.
