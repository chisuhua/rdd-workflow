...
## Purpose
This specification defines the MCP Client tools exposed to Spoke AI agents for communicating with the Hub MCP Server over the MCP protocol. The client provides four tools: `hub_read_issue`, `hub_create_issue`, `hub_update_status`, and `hub_sync_contract`. All tools SHALL return structured JSON conforming to defined schemas and SHALL record every invocation to the trace log.
