...
- [x] Task 1: python_version(str)— 来自 sys.version
- [x] Task 2: git_version(str)— 来自 git --version 或 None
- [x] Task 3: os_platform(str)— 来自 platform.platform()
- [x] Task 4: project_hash(str)— sha256(project_root)[:8],与 ADR §3 一致
- [x] Task 5: rddf_session_id(str | None)— 从 RDDF_SESSION_ID env 读
- [x] Task 6: skill_invoked(str)— 调用方传入,例如 rdd-doctor、guide-plan
- [x] Task 7: (no items specified)
