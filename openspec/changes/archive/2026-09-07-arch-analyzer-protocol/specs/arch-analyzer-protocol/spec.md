...
### Requirement: rdd-arch gap analysis must use verifier 5-section template verbatim
**Reason**: Naive full-template adoption (sections 3/4/5 for SHA-bound cache and staged context) is form-over-substance for a deterministic human-curated analyzer. The gap analysis has no LLM verdict, no agent context consumer — those sections would be empty files / no-op gates.

**Replacement**: Analyzer Subset pattern documented in `cross-stage-protocol-template.md` and `ADR-0046`. Adopts §1 (SKILL.md § Protocol) + §2 (Python data layer) + §5-repurposed (Output Contract Validation, non-blocking). Skips §3 (staged context) + §4 (SHA-bound cache) explicitly.

### Requirement: verifier-protocol-template.md as canonical name
**Reason**: The filename "verifier-protocol-template" misleads future adopters into thinking the pattern is verifier-only, when the spec's own Why section claims cross-stage intent. The rename is zero-cost (spec-only, no existing references).

**Replacement**: Renamed to `cross-stage-protocol-template.md` with Verifier Subset + Analyzer Subset sections making the applicability decision explicit at the document level.