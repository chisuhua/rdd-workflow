...
- **SSOT 单一来源**: category 字段从 `.rddf/improvements/<name>.md` 的 `**分类**:` 解析（移除 roadmap-meta.yaml fallback）
- **顺序保证**: read username → audit 写入 → hub re-fetch → accept（任一步失败则拒绝, audit 记录 `decision=fail`）
- **Offline 行为**: Hub re-fetch network 错误时 fail-open + warning（仅 network 类, 非 auth 类）
- **锁定**: 既有 4 个 bats 测试必须继续通过；新增 5 个 case 总计 9 个
- **CI 兼容**: `read -s` 在非交互终端需 fallback（用 `RDDF_APPROVE_ACTOR` env var）
- **审计完整性**: audit write 在 accept 前同步执行, crash-safety 由 atomic_write 模式保证
- **Bash 版本兼容**: 不引入 bash 4+ 特性（项目既有 bash 3.2+ 兼容约束）
- **rddf-session 集成**: ADR-0031 第 4 项引用 ADR-0017 §4 实际上是 State Machine 节（Oracle 发现误引），不作为本次修复 scope
