...
当前 `approve_proposal.sh --manual --hub-issue <org/repo#N>` 强制人工先创建 Hub Issue，再回填 URL。两步流程让人类在多个工具间跳转，且审批时看不到"准备发什么 RFC"。`rddf report-issue` 与 `approve_proposal.sh` 完全独立。
...
依赖 `add-cross-repo-impact-detection` + `add-rfc-draft-template`（Phase 1）。
