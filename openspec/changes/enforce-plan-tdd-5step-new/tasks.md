...
- [x] 1.1 Add `plan-tdd-check` function in `guide-ship/scripts/ship_plan.sh`
...
- [x] 1.2 Insert check call at start of `run_ship_phase1` / Phase 2 execute trigger
  - Before plan generation, run `check_plan_tdd "$PLAN_FILE"`
  - Exit 1 with user-facing message if check fails
- [x] 1.3 Implement `SKIP_PLAN_TDD_CHECK=yes` opt-out
...
- [x] 1.4 Add old-plan skip recommendation heuristic
...
- [x] 2.1 Create `tests/integration/test_guide_ship_plan_tdd_check.bats`
...
- [x] 2.2 Run `bats tests/integration/test_guide_ship_plan_tdd_check.bats` — all 4 cases pass
- [x] 2.3 Run `python3 -m pytest tests/unit/ -q --tb=short` — no regressions
- [x] 2.4 Run `openspec validate enforce-plan-tdd-5step-new` — passes
- [x] 2.5 End-to-end: ship a new change with `SKIP_PLAN_TDD_CHECK=yes`, verify audit entry created