...
- [x] 1.1 在 `ship_plan.sh::generate_implementation_plan()` 中用 `command -v skill_use >/dev/null 2>&1` 检测环境能力（而非依赖调用失败）
- [x] 1.2 无 `skill_use` 时输出明确指引："计划文件缺失，需编排者按 rdd-workflow-writing-plans 规范生成 .rddf/plans/<name>.md"，而非"技能未找到"错误
- [x] 1.3 降级时不返回非零退出码（当前 `return 1` 使 run_ship_phase1 失败），返回可辨识状态码
