...
- [ ] 1.1 给 `skills/_lib/schemas/*.json` 下 17 个 schema 文件统一添加顶层 `"version": {"const": "v1"}` 字段
...
- [ ] 1.2 新增 `tests/unit/test_schema_version_field.py`(≥17 用例)
...
- [ ] 1.3 给 `rdd-doctor` 新增 schema version self-check(`--category state`)
...
- [ ] 1.4 验证 `.rddf/state/` 下 state 文件通过新 schema 校验
...
- [ ] 1.5 文档化 `docs/schemas/README.md` 新增"version 字段语义"章节
...
- [ ] 1.6 同步更新 `tests/unit/test_cross_repo_schemas.py` 已有 schema 测试
  - 检查是否引用 schema 顶层 metadata 字段;如是则保持兼容(只读不写)
  - 不引入 regression: 现有测试用例保持 pass
- [ ] 1.7 手工验证: 退出码 0 命令必须全部成功
