...
- [ ] Task 1: .rddf/state/.issue-reporter.json state 文件
- [ ] Task 2: .rddf/state/.reporting-config.json 配置缓存
- [ ] Task 3: 一次性 banner 输出
- [ ] Task 4: _lib/schemas/issue_reporter_schema.json
- [ ] Task 5: close_on_archive / retention_days / redact_patterns 中 retention_days(保留 close_on_archive + redact_patterns)
- [ ] Task 6: 看到 supersession 注,提示设计已由 ADR-0029 替代
- [ ] Task 7: 不需要逐字读 ADR-0029;知道 §5 是设计历史,真正路径看 ADR-0029
- [ ] Task 8: 看到注释:retention_days 因 prunable code path 不可达,本 ADR 已删除承诺
