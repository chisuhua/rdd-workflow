...
- [x] 1.1 在 `_lib/archive.sh::archive_change` 末尾添加 `check_tasks_completion` 函数
...
- [x] 1.2 在 `archive_change` 函数 step 8.5 之前(line 397)插入 hook 调用
...
- [x] 1.3 实现 `STRICT_TASKS_GATE=yes` escalation
...
- [x] 1.4 实现 `SKIP_TASKS_GATE=yes` opt-out
...
- [x] 1.5 给 `rdd-doctor` 实现 `--category tasks-checkbox` 检查
...
- [x] 1.6 新增 `tests/integration/test_archive_tasks_gate.bats`(≥5 用例)
...
- [x] 1.7 验证现有 archive 相关测试保持 pass(无 regression)
