...
- **格式支持**：仅 OpenAPI 3.0+ / Protobuf 3+（其他格式后续 ADR 评估）
- **差异分级**：Breaking-Change / Non-Breaking / New-Contract 三级
- **缓存友好**：契约版本缓存在 `.rddf/state/.contract-cache.json`
- **离线模式**：Hub 不可达时使用本地缓存 + 警告（不阻断）
