#!/usr/bin/env bash
# skills/guide-design/scripts/design_proposal_review.sh
...
#
# 用法：
#   source "$(dirname "${BASH_SOURCE[0]}")/scripts/design_proposal_review.sh"
#   design_proposal_review "$PROJECT_ROOT" "$PHASE_5_5_ENTRY"
#   rc=$?  # 0=正常完成, 1=返回上级
#
# 参数：
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  design_proposal_review "${1:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}" "${2:-phase1}"
fi
