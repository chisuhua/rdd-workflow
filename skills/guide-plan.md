...
plan → ship 交接通过 `.rddf/state/.plan-handoff.json` 软状态文件传递。plan-done 验证通过后立即写入，记录 plan_complete_at、active_changes、all_artifacts_committed、ship_started_at（初值 null）、current_change（第一个 active change 名）。文件不被 git 跟踪（`.gitignore` 已排除 `.rddf/`），缺失时 ship 端静默回退到旧行为。

```bash
# 写入 handoff 状态,作为 plan→ship 的软交接信号
# 缺失 .rddf/state 目录时静默创建 (mkdir -p),写失败不阻塞 plan-done 输出
...
if [ -f "$HANDOFF_FILE" ]; then
    echo "✅ Handoff state written: .rddf/state/.plan-handoff.json (active_changes=$CHANGE_COUNT, current_change=$CURRENT_CHANGE)"
else
