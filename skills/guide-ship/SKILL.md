...
review 阶段的目的是分类记录债务，不阻断 archive。如果用户选择 "跳过"（选项 4），正常进入 Phase 3。门控检查通过 gate.py 的 `review_debt_recorded`（warning 级别）实现，见 `_lib/gate.py`。
...
**脚本入口**: `_lib/ship_execution_mode.sh`

```bash
# 解析模式
source _lib/ship_execution_mode.sh
