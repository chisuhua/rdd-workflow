...
- **状态持久化**：arch-done 时写入 `.zcf/.arch-handoff.json`（不被 git 跟踪，缺失时 plan 端静默回退）
...
[guide-arch]  --(arch-done: ADR ≥ 1 + roadmap.md)-->  [guide-plan]
   ...
   exits: .zcf/.arch-handoff.json                     exits: .zcf/.plan-handoff.json
```
...
- arch-done 时写入 `.zcf/.arch-handoff.json`（arch → plan 的软交接信号）
...
最新 ADR:
  ...
  - ADR-0001: spec-workflow 状态机分相 [已替代为 ADR-0002+0003]
...
# P1-6 兼容模式检测：当 roadmap.md 不存在但 .zcf/.roadmap-state.json 仍存在
...
arch → plan 交接通过 `.zcf/.arch-handoff.json` 软状态文件传递。arch-done 验证通过后立即写入，记录 arch_complete_at、adr_count、roadmap_exists、plan_started_at（初值 null）。文件不被 git 跟踪（`.gitignore` 排除 `.zcf/`），缺失时 plan 端静默回退到旧行为。

```bash
# P2-5 模式: 写入 handoff 状态,作为 arch→plan 的软交接信号
# 缺失 .zcf 目录时静默创建 (mkdir -p),写失败不阻塞 arch-done 输出
HANDOFF_FILE="$PROJECT_ROOT/.zcf/.arch-handoff.json"
...
if [ -f "$HANDOFF_FILE" ]; then
    echo "✅ Handoff state written: .zcf/.arch-handoff.json (adr_count=$ADR_COUNT, roadmap_exists=true)"
else
