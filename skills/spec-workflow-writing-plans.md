---
name: spec-workflow/writing-plans
description: 自包含的 OpenSpec 实施计划生成器。为 OpenSpec change 生成 TDD 5 步结构的实施计划(.rddf/plans/<name>.md)。基于 superpowers/writing-plans 改写,完全自包含于 spec-workflow,不依赖任何外部 skill。被 guide-ship 在 Phase 1 plan 阶段调用。
license: MIT
...
# Writing Plans (spec-workflow/writing-plans)
...
**调用方式**:
```
skill_use("spec-workflow/writing-plans")   # 无参数,依赖 git context 获取 CHANGE_NAME / WT_PATH
```
