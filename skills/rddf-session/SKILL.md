...
if current:
    ...
    if nxt:
        ...
        print("   No orphaned rddf-sessions found. Run rdd-arch or rdd-builder to start.")
    PYEOF
        ;;
...
- **Heartbeat**: refreshed on every `rdd-arch`/`rdd-planner`/`rdd-builder` phase call;
...
- `rdd-arch` / `rdd-planner` / `rdd-builder` / `rdd-verifier` automatically create `kind=stage_*` rddf-sessions on entry and close them on phase completion.
