# Backward-compat shim for old `skills/_lib/archive.sh` path.
# Re-sources the new top-level `_lib/archive.sh` so existing code blocks and
...
if [ -f "${_PARENT_DIR}/_lib/archive.sh" ]; then
  source "${_PARENT_DIR}/_lib/archive.sh"
else
  source "${HOME:?}/.agents/skills/_lib"/archive.sh
fi
