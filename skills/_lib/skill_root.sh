...
resolve_rdd_lib_dir() {
    ...
    if [ -n "${RDD_WORKFLOW_SRC:-}" ] && [ -d "$RDD_WORKFLOW_SRC/skills/_lib" ]; then
        echo "$RDD_WORKFLOW_SRC/skills/_lib"
        return 0
    fi
    echo "ERROR: Cannot resolve _lib dir" >&2
    echo "  Searched: \$PROJECT_ROOT/.opencode/skills/_lib, ~/.agents/skills/_lib, \$RDD_WORKFLOW_SRC/skills/_lib" >&2
    return 1
}
