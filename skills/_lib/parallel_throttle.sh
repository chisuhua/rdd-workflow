# Backward-compat shim for old `skills/_lib/parallel_throttle.sh` path.
# Re-sources the new top-level `_lib/parallel_throttle.sh` so existing code blocks and
...
if [ -f "${_PARENT_DIR}/_lib/parallel_throttle.sh" ]; then
  source "${_PARENT_DIR}/_lib/parallel_throttle.sh"
else
  source "${HOME:?}/.agents/skills/_lib"/parallel_throttle.sh
fi
