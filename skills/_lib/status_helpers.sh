# Backward-compat shim for old `skills/_lib/status_helpers.sh` path.
# Re-sources the new top-level `_lib/status_helpers.sh` so existing code blocks and
...
if [ -f "${_PARENT_DIR}/_lib/status_helpers.sh" ]; then
  source "${_PARENT_DIR}/_lib/status_helpers.sh"
else
  source "${HOME:?}/.agents/skills/_lib"/status_helpers.sh
fi
