...
> 上面的 `description` 是一个 JSON 字符串字段（用 `\n` 表示换行）。`propose.md` 和 4 个其他 consumer（`guide-design.md`、`guide.md`、`status.md`、`deps.md`）都通过 `_lib/state.sh::read_suggestions` / `write_suggestions` 读写。
...
if [ -f "$PROJECT_ROOT/openspec/changes/<name>/.openspec.yaml" ]; then
    if ! python3 "$(resolve_rdd_skill_dir propose)/scripts/validate_baseline.py" "<name>" >/dev/null 2>&1; then
