...
**结构化输出 (deps-analysis.json)**：与 `iteration.json` 并列在 `.rddf/state/` 下的机器可读快照，schema 在 `_lib/schemas/deps_analysis_schema.json`。下游消费者（未来的 planner、status 增强）应优先读此 JSON，markdown 仅用于人类阅读。
